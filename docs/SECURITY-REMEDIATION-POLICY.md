...
- **Rotation:** the maintainer's GitHub credentials use the platform's
  strong-auth features; a leaked or suspected-leaked credential is rotated
  immediately and the incident is noted in the affected release notes if any
  artifact could have been touched.
