...
# --------------------------------------------------------------------------------------------------
# Zitadel is the only such provider: its first instance, org and a machine account with a personal access
# token come from environment, but the project, the OIDC application, the role and the user grants exist only
# through its management API — and the client id and secret are GENERATED by that call, so they can only be
# known after seeding. It runs HERE, before the signing assertion below, because the same seeding turns on
# the feature that makes Zitadel publish a signing key at all. Every other harness declares its client
# statically and this is a no-op.
idp_seed() {
  [ "$IDP_KIND" = "zitadel" ] || return 0
